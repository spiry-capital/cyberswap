...
contract Cyberswap is ERC20, ReentrancyGuard, Ownable {
        ...
        returns (uint256 fairSupply)
    {
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            ...
            require(
                amount >= minAmounts[i],
                "Cyberswap: minAmount not reached"
            );
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            require(
                ...
                "Cyberswap: result is not enough"
            );
    ...
    function swap(
        ...
        require(
            ...
            "Cyberswap: wrong value usage"
        );

        Balances memory balances =
            Balances({
                src: src.uniBalanceOf(address(this)).sub(
        ...
        require(
            ...
            "Cyberswap: return is not enough"
        );
        ...
        address targetReferral = address(0);

        if (referral != address(0)) {
            targetReferral = referral;
        } else if (devAddress != address(0)) {
            targetReferral = devAddress;
        }

        if (targetReferral != address(0)) {
...
// File: contracts\CyberFactory.sol
